...
class Form
{
    ...
    public function addSelect(string $name, array $options, $selectedValue, array $attributes = []): self
    {
        ...
        foreach ($options as $key => $value) {
            // Vérifie si l'option correspond à la valeur sélectionnée
    ...
    public function addImage(string $name, string $src, array $attributes = []): self
    {
        $imgAttributes = '';
        foreach ($attributes as $attribute => $value) {
            $imgAttributes .= "$attribute=\"$value\" ";
        }
    
        $img = "<img src=\"$src\" $imgAttributes/>";
